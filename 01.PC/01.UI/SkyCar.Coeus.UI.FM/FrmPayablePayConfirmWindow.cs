...
namespace SkyCar.Coeus.UI.FM
{
    ...
    /// 应付单确认付款弹出窗
    /// </summary>
    public partial class FrmPayablePayConfirmWindow : BaseForm
    {
        #region 全局变量
        ...
        /// 应付管理BLL
        /// </summary>
        private PayablePayConfirmWindowBLL _bll = new PayablePayConfirmWindowBLL();
        ...
        /// Grid的数据源
        /// </summary>
        List<PayablePayConfirmUIModel> ListGridDS = new List<PayablePayConfirmUIModel>();
        ...
        /// 应付单确认付款弹出窗
        /// </summary>
        public FrmPayablePayConfirmWindow()
        {
            InitializeComponent();
        }
        /// <summary>
        ...
        public FrmPayablePayConfirmWindow(Dictionary<string, object> paramWindowParameters)
        {
            ...
            for (int i = 0; i < ListGridDS.Count; i++)
            {
                ...
                if (ListGridDS[i].UnPayTotalAmount < 0)
                {
                    if (thisPayAmount > 0 && i == (ListGridDS.Count - 1))
                    {
                        ...
                        {
                            if (ListGridDS[j].PayableTotalAmount > 0)
                            {
                                ListGridDS[j].ThisPayAmount = ListGridDS[j].ThisPayAmount + thisPayAmount;
                                thisPayAmount = 0;
                                break;
                            }
                        }
                ...
                if (thisPayAmount > 0 && i == (ListGridDS.Count - 1))
                {
                    for (int j = 0; j < ListGridDS.Count; j++)
                    {
                        if (ListGridDS[j].APB_AccountPayableAmount > 0)
                        {
                            ListGridDS[j].ThisPayAmount = ListGridDS[j].ThisPayAmount + thisPayAmount;
                            thisPayAmount = 0;
                            break;
        ...
        private void cbPayTypeName_ValueChanged(object sender, EventArgs e)
        {
            ...
            else if (cbPayTypeName.Text == TradeTypeEnum.Name.CASH)
            {
        ...
        private void gdGrid_CellChange(object sender, CellEventArgs e)
        {
            ...
            if (e.Cell.Column.Key == "ThisPayAmount")
            {
                ...
                foreach (var loopGrid in ListGridDS)
                {
                    if (loopGrid.UnPayTotalAmount < 0)
                    {
                        loopGrid.ThisPayAmount = loopGrid.UnPayTotalAmount;
                    }
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            ...
            if (!_bll.SavePayablePayConfirmData(_payBillManager, ListGridDS))
            {
                //保存失败
