...
namespace SkyCar.Coeus.UI.Common
{
    ...
    /// 车辆车系查询
    /// </summary>
    public partial class FrmVehicleInspireQuery : BaseForm
    {
        #region 全局变量
        /// <summary>
        /// 车辆品牌车系查询BLL
        ...
        /// Grid的数据源
        /// </summary>
        List<VehicleInspireQueryUIModel> ListGridDS = new List<VehicleInspireQueryUIModel>();
        ...
        public FrmVehicleInspireQuery(string paramSelectedValuePropertyName,
        ...
        /// <param name="e"></param>
        private void FrmVehicleInspireQuery_Load(object sender, EventArgs e)
        {
            #region 固定
            //基类.工具栏（翻页）
        ...
        {
            _bll.QueryForList<VehicleInspireQueryUIModel>(SQLID.COMM_SQL10, new VehicleInspireQueryQCModel()
            {
                PageSize = PageSize,
                PageIndex = PageIndex,
                //车辆品牌
                WHERE_VBIS_Brand = txtWhere_VBIS_Brand.Text.Trim(),
                //车系
                WHERE_VBIS_Inspire = txtWhere_VBIS_Inspire.Text.Trim()
            }, ListGridDS);
                ...
                foreach (var loopSourceItem in ListGridDS)
                {
                    ...
                    foreach (PropertyInfo pi in tp.GetProperties())
                    {
                        ...
                        if (pi.Name == SelectedValuePropertyName)
                        {
                            ...
                            if (
                                !string.IsNullOrEmpty(tempSelectedValue))
                            {
        ...
        private void InitializeListTabControls()
        {
            #region 查询条件初始化
            //车辆品牌
            txtWhere_VBIS_Brand.Clear();
            //车辆车系
            txtWhere_VBIS_Inspire.Clear();
            //给 车辆品牌 设置焦点
            lblWhere_VBIS_Inspire.Focus();

            #endregion

            //清空Grid
            ListGridDS = new List<VehicleInspireQueryUIModel>();
            gdGrid.DataSource = ListGridDS;
            gdGrid.DataBind();

            //设置父类别
            txtWhere_VBIS_Brand.Text = SelectedItemParentValue;
            txtWhere_VBIS_Brand.ReadOnly = !string.IsNullOrEmpty(SelectedItemParentValue);
            txtWhere_VBIS_Brand.Enabled = string.IsNullOrEmpty(SelectedItemParentValue);
        }
