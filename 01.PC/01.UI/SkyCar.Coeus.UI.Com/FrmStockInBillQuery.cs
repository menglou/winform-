...
namespace SkyCar.Coeus.UI.Common
{
    ...
    /// 入库单查询
    /// </summary>
    public partial class FrmStockInBillQuery : BaseForm
    {
        #region 全局变量
        /// <summary>
        /// 入库单查询BLL
        ...
        /// Grid的数据源
        /// </summary>
        List<StockInBillQueryUIModel> ListGridDS = new List<StockInBillQueryUIModel>();

        /// <summary>
        /// 接收SourceView的Func
        /// </summary>
        private Func<StockInBillQueryUIModel, bool> _salesOrderDetailFunc;
        ...
        /// 来源类型数据源
        /// </summary>
        List<ComComboBoxDataSourceTC> _stockInBillSourceTypeList = new List<ComComboBoxDataSourceTC>();
        /// <summary>
        /// 审核状态数据源
        /// </summary>
        ...
        /// 选中项列表
        /// </summary>
        public List<StockInBillQueryUIModel> SelectedGridList = new List<StockInBillQueryUIModel>();
        #endregion
        ...
        /// <param name="e"></param>
        private void FrmStockInBillQuery_Load(object sender, EventArgs e)
        {
            #region 固定
            //基类.工具栏（翻页）
            ...
            if (_viewParameters != null)
            {
                ...
                if (_viewParameters.ContainsKey(ComViewParamKey.ApprovalStatus.ToString()))
                {
                    ...
                    if (_approvalStatusList != null && _approvalStatusList.Count > 0)
                    {
                        ...
                        cbWhere_SIB_ApprovalStatusName.Text = _approvalStatusList[0].Text;
                        cbWhere_SIB_ApprovalStatusName.Value = _approvalStatusList[0].Code;

                        if (_approvalStatusList.Count == 1)
                        {
                            cbWhere_SIB_ApprovalStatusName.Enabled = false;
                        }
                        else
                        {
                            cbWhere_SIB_ApprovalStatusName.Enabled = true;
                        }
                    }
                }
                #endregion
                ...
                if (_viewParameters.ContainsKey(ComViewParamKey.SourceType.ToString()))
                {
                    if (_viewParameters.ContainsKey(ComViewParamKey.SourceType.ToString()))
                    {
                        _stockInBillSourceTypeList = _viewParameters[ComViewParamKey.SourceType.ToString()] as List<ComComboBoxDataSourceTC>;
                        if (_stockInBillSourceTypeList != null && _stockInBillSourceTypeList.Count > 0)
                        {
                            ...
                            cbWhere_SIB_SourceTypeName.Text = _stockInBillSourceTypeList[0].Text;
                            cbWhere_SIB_SourceTypeName.Value = _stockInBillSourceTypeList[0].Code;

                            if (_stockInBillSourceTypeList.Count == 1)
                            {
                                cbWhere_SIB_SourceTypeName.Enabled = false;
                            }
                            else
                            {
                                cbWhere_SIB_SourceTypeName.Enabled = true;
                            }
                        }
                    }
                }
        ...
        private void btnClearSelectedItem_Click(object sender, EventArgs e)
        {
            ...
            this.gdGrid.Refresh();
            SelectedGridList = new List<StockInBillQueryUIModel>();
        }
        ...
        {
            _bll.QueryForList<StockInBillQueryUIModel>(SQLID.COMM_SQL37, new StockInBillQueryQCModel()
            {
                PageSize = PageSize,
                PageIndex = PageIndex,
                //组织
        ...
        void GenerateSelectedValueAndText()
        {
            ...
            foreach (var loopSourceItem in ListGridDS)
            {
                ...
                StockInBillQueryUIModel argsStockInBill = new StockInBillQueryUIModel
                {
                    SIB_ID = loopSourceItem.SIB_ID,
                    SIB_Org_ID = loopSourceItem.SIB_Org_ID,
                    SIB_No = loopSourceItem.SIB_No,
                    SIB_SourceTypeCode = loopSourceItem.SIB_SourceTypeCode,
                    SIB_SourceTypeName = loopSourceItem.SIB_SourceTypeName,
                    SIB_SourceNo = loopSourceItem.SIB_SourceNo,
                    SIB_StatusCode = loopSourceItem.SIB_StatusCode,
                    SIB_StatusName = loopSourceItem.SIB_StatusName,
                    SIB_ApprovalStatusCode = loopSourceItem.SIB_ApprovalStatusCode,
                    SIB_ApprovalStatusName = loopSourceItem.SIB_ApprovalStatusName,
                    SIB_IsValid = loopSourceItem.SIB_IsValid,
                    SIB_CreatedBy = loopSourceItem.SIB_CreatedBy,
                    SIB_CreatedTime = loopSourceItem.SIB_CreatedTime,
                    SIB_UpdatedBy = loopSourceItem.SIB_UpdatedBy,
                    SIB_UpdatedTime = loopSourceItem.SIB_UpdatedTime,
                    SIB_VersionNo = loopSourceItem.SIB_VersionNo,

                    //供应商
                    SID_SUPP_ID = loopSourceItem.SID_SUPP_ID,
                    SUPP_Name = loopSourceItem.SUPP_Name,
                };
                SelectedGridList.Add(argsStockInBill);
            }
        }
