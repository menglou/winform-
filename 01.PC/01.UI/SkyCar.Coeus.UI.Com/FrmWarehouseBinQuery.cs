...
namespace SkyCar.Coeus.UI.Common
{
    ...
    /// 仓位查询
    /// </summary>
    public partial class FrmWarehouseBinQuery : BaseForm
    {
        #region 全局变量
        /// <summary>
        /// 仓位查询BLL
        ...
        /// Grid的数据源
        /// </summary>
        List<WarehouseBinQueryUIModel> ListGridDS = new List<WarehouseBinQueryUIModel>();
        ...
        /// <param name="e"></param>
        private void FrmWarehouseBinQuery_Load(object sender, EventArgs e)
        {
            #region 固定
            //基类.工具栏（翻页）
        ...
        {
            _bll.QueryForList<WarehouseBinQueryUIModel>(SQLID.COMM_SQL16, new WarehouseBinQueryQCModel()
            {
                PageSize = PageSize,
                PageIndex = PageIndex,
                //仓库名称
                ...
                //仓位
                WHERE_WHB_Name = txtWhere_WHB_Name.Text.Trim()
            }, ListGridDS);
        ...
        private void InitializeListTabControls()
        {
            ...
            //仓位
            txtWhere_WHB_Name.Clear();
            //给 仓位 设置焦点
            lblWhere_WHB_Name.Focus();

            #endregion

            //清空Grid
            ListGridDS = new List<WarehouseBinQueryUIModel>();
            gdGrid.DataSource = ListGridDS;
            gdGrid.DataBind();

            //设置父类别
            txtWhere_WH_Name.Text = SelectedItemParentValue;
            txtWhere_WH_Name.ReadOnly = !string.IsNullOrEmpty(SelectedItemParentValue);
            txtWhere_WH_Name.Enabled = string.IsNullOrEmpty(SelectedItemParentValue);
        }
