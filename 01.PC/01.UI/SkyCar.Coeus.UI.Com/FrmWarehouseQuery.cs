...
namespace SkyCar.Coeus.UI.Common
{
    ...
    /// 仓库查询
    /// </summary>
    public partial class FrmWarehouseQuery : BaseForm
    {
        #region 全局变量
        /// <summary>
        /// 仓库查询BLL
        ...
        /// Grid的数据源
        /// </summary>
        List<WarehouseQueryUIModel> ListGridDS = new List<WarehouseQueryUIModel>();
        ...
        public FrmWarehouseQuery(string paramSelectedValuePropertyName,
            ...
            string paramItemSelectedItemParentText = null)
        {
            ...
            //paramItemSelectedItemParentText为空时，用paramItemSelectedItemParentValue代替
            if (string.IsNullOrEmpty(paramItemSelectedItemParentText))
            {
                //选中项的父级描述
                SelectedItemParentText = paramItemSelectedItemParentValue;
            }
            else
            {
                //选中项的父级描述
        ...
        /// <param name="e"></param>
        private void FrmWarehouseQuery_Load(object sender, EventArgs e)
        {
            #region 固定
            //基类.工具栏（翻页）
        ...
        {
            _bll.QueryForList<WarehouseQueryUIModel>(SQLID.COMM_SQL15, new WarehouseQueryQCModel()
            {
                PageSize = PageSize,
                PageIndex = PageIndex,
                //仓库名称
                ...
                //组织ID
                WHERE_WH_Org_ID = txtWhere_Org_Name.Tag.ToString(),
                //组织名称
                WHERE_Org_ShortName = txtWhere_Org_Name.Text.Trim()
            }, ListGridDS);
        ...
        private void InitializeListTabControls()
        {
            ...
            //清空Grid
            ListGridDS = new List<WarehouseQueryUIModel>();
            gdGrid.DataSource = ListGridDS;
            gdGrid.DataBind();

            //设置父类别
            txtWhere_Org_Name.Text = SelectedItemParentText;
            txtWhere_Org_Name.Tag = !string.IsNullOrEmpty(SelectedItemParentValue) ? SelectedItemParentValue : string.Empty;
            txtWhere_Org_Name.ReadOnly = !string.IsNullOrEmpty(SelectedItemParentValue);
            txtWhere_Org_Name.Enabled = string.IsNullOrEmpty(SelectedItemParentValue);
        }
