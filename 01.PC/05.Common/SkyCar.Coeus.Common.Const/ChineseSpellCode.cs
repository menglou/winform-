...
{
    public class ChineseSpellCode
    {
        #region 中文首拼音
        /// <summary>
        /// 汉字拼音首字母列表 本列表包含了20902个汉字,用于配合 GetChineseSpellCode方法使用,本表收录的字符的Unicode编码范围为19968至40869
        /// </summary>
        private static readonly StringBuilder ChineseFirstSpellCode = new StringBuilder(
        ...
        /// <summary>
        ///  获得一个字符串的汉语拼音码(如：拼音，PY)
        /// </summary>
        /// <param name="chineseText">字符串</param>
        /// <returns>汉语拼音码,该字符串只包含大写的英文字母</returns>
        public static string GetShortSpellCode(string chineseText)
        {
            ...
            foreach (var loopSpecialShortSpellCode in SpecialShortSpellCodeList)
            {
                if (chineseText.Contains(loopSpecialShortSpellCode.Key))
                {
                    chineseText = chineseText.Replace(loopSpecialShortSpellCode.Key, loopSpecialShortSpellCode.Value);
                }
            }

            StringBuilder myStr = new StringBuilder();
            foreach (char vChar in chineseText)
            {
                // 若是字母或数字则直接输出
                if ((vChar >= 'a' && vChar <= 'z')
                    || (vChar >= 'A' && vChar <= 'Z')
                ...
                else
                {
                    ...
                    if (index >= 0 && index < ChineseFirstSpellCode.Length)
                    {
                        myStr.Append(ChineseFirstSpellCode[index]);
                    }
                }
            }
        ...
        public static string GetFirstSpellCode(string chineseText)
        {
            ...
            char vChar = (chineseText.ToCharArray())[0];
            ...
            {
                // 对可以查找的汉字计算它的首拼音字母的位置，然后输出
                myStr = ChineseFirstSpellCode[vChar - 19968].ToString();
            }
        ...
        /// <summary>
        /// 为字符串加上汉语拼音码的第一个字母(如：拼音，P拼音)
        /// </summary>
        /// <param name="chineseText"></param>
        /// <returns></returns>
        public static string AddFirstPinyin(string chineseText)
        {
            if (string.IsNullOrEmpty(chineseText))
            {
                return string.Empty;
            ...
            {
                // 对可以查找的汉字计算它的首拼音字母的位置，然后输出
                string strNew = ChineseFirstSpellCode[vChar - 19968].ToString();
                return strNew + chineseText;
            }
            else
            {
                return chineseText;
        ...
        /// <summary> 
        /// 把汉字转换成拼音(全拼) (如：拼音，PINYIN)
        /// </summary>
        /// <param name="chineseText">汉字字符串</param>
        /// <returns>转换后的拼音(全拼)字符串</returns> 
        public static string GetFullSpellCode(string chineseText)
        {
            if (string.IsNullOrEmpty(chineseText))
            {
                return chineseText;
            ...
            foreach (var loopSpecialFullSpellCode in SpecialFullSpellCodeList)
            {
                if (chineseText.Contains(loopSpecialFullSpellCode.Key))
                {
                    chineseText = chineseText.Replace(loopSpecialFullSpellCode.Key, loopSpecialFullSpellCode.Value);
                }
            }
            ...
            char[] noWChar = chineseText.ToCharArray();

            foreach (char vChar in noWChar)
            {
                // 中文字符 
                if (regex.IsMatch(vChar.ToString()))
                {
                    var array = Encoding.Default.GetBytes(vChar.ToString());
                    int i1 = array[0];
                    int i2 = array[1];
        ...
        /// 特殊的首拼
        /// </summary>
        public static Dictionary<string, string> SpecialShortSpellCodeList = new Dictionary<string, string>
        {
            {"刹车","SC" },
            {"空调","KT" },
            {"长安","CA" },
        ...
        /// 特殊的全拼
        /// </summary>
        public static Dictionary<string, string> SpecialFullSpellCodeList = new Dictionary<string, string>
        {
            {"刹车","ShaChe" },
            {"空调","KongTiao" },
            {"长安","ChangAn" },
