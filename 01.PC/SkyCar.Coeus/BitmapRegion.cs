...
{
    /// <summary> 
    /// Summary description for BitmapRegion.(不规则登陆窗体用)
    /// </summary> 
    public class BitmapRegion
    {
        ...
        public static void CreateControlRegion(Control control, Bitmap bitmap)
        {
            ...
            //设置控件大小为位图大小
            control.Width = bitmap.Width+150;
            control.Height = bitmap.Height+100;
        ...
        private static GraphicsPath CalculateControlGraphicsPath(Bitmap bitmap)
        {
            ...
            for (int row = 0; row < bitmap.Height; row++)
            {
                ...
                for (int col = 0; col < bitmap.Width; col++)
                {
                    ...
                    if ((Math.Abs(bitmap.GetPixel(col, row).R - colorTransparent.R) > 5)
                        && (Math.Abs(bitmap.GetPixel(col, row).G - colorTransparent.G) > 5)
