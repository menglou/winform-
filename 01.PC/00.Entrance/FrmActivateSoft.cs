...
{
    public partial class FrmActivateSoft : BaseForm
    {
        ...
        private void ValActivateSoftCode(string activateCode)
        {
            ...
            if (jsonResult[SysConst.EN_RESULTCODE].ToString().Equals(SysConst.EN_I0001))
            {
                ...
                try
                {
                    ...
                    if (argsListOrganization != null)
                    {
                        ...
                        bll.QueryForList<MDLSM_Organization>(new MDLSM_Organization(), localOrgList);
                        ...
                        if (localOrgList.Count > 0)
                        {
                            foreach (var model in localOrgList)
                            {
                                ...
                                if (!boolTemp)
                                {
                                    MessageBoxs.Show(Trans.COM, this.ToString(), "商户激活失败！\r\n更新组织状态失败！", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        ...
                        foreach (var org in plateformOrgList)
                        {
                            ...
                            if (templocalOrgList.Count > 0)
                            {
                                ...
                                if (!boolTemp)
                                {
                                    MessageBoxs.Show(Trans.COM, this.ToString(), "商户激活失败！\r\n更新组织内容失败！", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    DBManager.RollBackTransaction(DBCONFIG.Coeus);
                                    return;
                                }
                            }
                            else
                            {
                                ...
                                if (!boolTemp)
                                {
                                    MessageBoxs.Show(Trans.COM, this.ToString(), "商户激活失败！\r\n新增组织失败！", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    DBManager.RollBackTransaction(DBCONFIG.Coeus);
                                    return;
                                }
                            }
                    ...
                    bll.QueryForObject<MDLSM_Parameter, MDLSM_Parameter>(argsParameterQuery, parameterResult);
                    ...
                    if (!boolTemp)
                    {
                        MessageBoxs.Show(Trans.COM, this.ToString(), "商户激活失败！\r\n更新系统参数（商户编码）失败！", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        DBManager.RollBackTransaction(DBCONFIG.Coeus);
                        return;
                    }
                    #endregion
            ...
            else
            {
                ...
                MessageBoxs.Show(Trans.COM, this.ToString(), "商户激活失败！\r\n" + strErrorMessage, MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
